...
public class StationsDBAdapter implements Runnable {
	...
	public StationsDBAdapter(Context ctx, MapView mapView, Handler handler, StationOverlayList stationsDisplayList){
		this.mCtx = ctx;
	...
	public void run() {
		while (!toDo.isEmpty()){
			...
			case FETCH:
				try{
					RAWstations = fetchStations(threadData.getString(STATION_PROVIDER_KEY));
			...
			case UPDATE_MAP_LESS:
				try{
					GeoPoint center = new GeoPoint(threadData.getInt(CENTER_LAT_KEY),threadData.getInt(CENTER_LNG_KEY));
